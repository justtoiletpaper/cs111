...
def match_time(text):
    ...
    ['2:00']
    """
    return re.findall(r'\b(?:(?:0?[0-9])|(?:1[0-9])|(?:2[0-4])):(?:(?:[0-5][0-9])|(?:60))[AP]?M?', text)
...
def area_codes(text):
    ...
    []
    """
    numbers = re.findall(r'\(?\d{3}\)?[ -]?\d{3}[ -]?\d{4}\b', text)
    print(numbers)
    num_str = ''
    for number in numbers:
        rep = number.replace(' ', '').replace('(', '').replace(')', '').replace('-', '')
        num_str += rep + ' '
    num_str.strip()
    print(num_str)
    return re.findall(r'\b\d{3}', num_str)
...
def most_common_code(text):
    ...
    def appearances(lst):
        lst_dict = {}
        for num_str in lst:
            if num_str not in lst_dict:
                lst_dict[num_str] = 1
            else:
                lst_dict[num_str] += 1
        return lst_dict

    def dict_counter(dictionary):
        highest_count = 0
        for num in dictionary:
            if dictionary[num] > highest_count:
                highest_count = dictionary[num]
        highest_count_lst = []
        for num in dictionary:
            if dictionary[num] == highest_count:
                highest_count_lst.append(num)
        return highest_count_lst[0]

    area_code_lst = area_codes(text)
    area_codes_dict = appearances(area_code_lst)
    return dict_counter(area_codes_dict)
